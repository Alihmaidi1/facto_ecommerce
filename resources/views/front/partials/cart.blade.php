<a href="javascript:void(0)" class="d-flex align-items-center text-reset h-100" data-toggle="dropdown"
    ...
    <span class="flex-grow-1 ml-1">
        @if (0)
            <span style="background:rgb(233, 4, 4)" class="badge badge-primary badge-inline badge-pill cart-count">
                {{ count($cart) }}
            </span>
        @else
            <span style="background:rgb(233, 4, 4)" class="badge badge-primary badge-inline badge-pill cart-count">0</span>
        @endif
        <span class="nav-box-text d-none d-xl-block opacity-70">{{ __('Cart') }}</span>
    </span>
</a>
<div class="dropdown-menu dropdown-menu-right dropdown-menu-lg p-0 stop-propagation">
    ...
    @if (isset($cart) && count($cart) > 0)
        ...
        <div class="px-3 py-2 fs-15 border-top d-flex justify-content-between">
            <span class="opacity-60">{{ __('Subtotal') }}</span>
    ...
    @else
        <div class="text-center p-3">
            ...
            <h3 class="h6 fw-700">{{ __('Your Cart is empty') }}</h3>
        </div>
    @endif
